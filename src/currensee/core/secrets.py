...
class SecretManager:
    ...
    def __init__(self):
        ...
        if not self._project_id:
            ...
            try:
                ...
                raise ValueError("No .env file found in curensee repository.")
    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.error(
                    "Could not authenticate with Google Cloud. Make sure your credentials are properly set up."
                )
    ...
    def get_secret(self, secret_id: str, version_id: str = "latest") -> Optional[str]:
        ...
        if self._google_auth_available and self.client is not None:
            ...
            try:
                logger.info(
                    f"Attempting to retrieve {secret_id} from Google Secret Manager"
                )
                name = f"projects/{self._project_id}/secrets/{secret_id}/versions/{version_id}"
                response = self.client.access_secret_version(request={"name": name})
                secret_value = response.payload.data.decode("UTF-8")
                logger.info(
                    f"Successfully retrieved {secret_id} from Google Secret Manager"
                )
            except Exception as e:
                logger.warning(
                    f"Error accessing secret {secret_id} from Secret Manager: {e}"
                )
                # Mark Google Auth as unavailable for certain errors
                if "PERMISSION_DENIED" in str(e) or "UNAUTHENTICATED" in str(e):
                    logger.error(
                        "Authentication issue with Google Secret Manager. Disabling for this session."
                    )
                    self._google_auth_available = False
        else:
            logger.warning(
                f"Skipping Google Secret Manager for {secret_id} due to previous auth failures"
            )
